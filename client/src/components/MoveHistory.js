...
const MoveHistory = ({ history }) => {
  ...
  for (let i = 0; i < history.length; i += 2) {
    movePairs.push({ number: Math.floor(i / 2) + 1, white: history[i], black: history[i + 1] || null });
  }
  ...
  return (
    <div className="bg-gray-800 rounded-lg p-4 shadow-lg w-64">
      ...
      <div
        ref={scrollRef}
        className="max-h-80 overflow-y-auto space-y-1 font-mono text-sm move-history-scroll"
      >
        {movePairs.length === 0 ? (
          ...
          movePairs.map((pair, idx) => (
            <div
              key={pair.number}
              className={`flex items-center py-1 px-2 rounded ${idx === movePairs.length - 1 ? 'bg-gray-700' : ''}`}
            >
      ...
      {history.length > 0 && (
        <div className="mt-4 pt-2 border-t border-gray-600">
          <div className="text-gray-400 text-xs">Total moves: {history.length}</div>
        </div>
      )}
    </div>
  );
