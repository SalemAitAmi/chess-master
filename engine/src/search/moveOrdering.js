 ...
 *
 *   TT_MOVE          2_000_000
 *   BOOK_MOVE        1_500_000   (+1000 for the heaviest pick — see search.js)
 ...
 *
 * Layout: imports → module constants/tables → pure helpers → classes.
 */
...
const HISTORY_BONUS_CAP = 400;
const PHASE_MAX         = 24;

// ── Soft-pin board-state tables ──
// Per-file and long-diagonal masks, used to decide whether a frozen rook or a
// frozen bishop costs more in the current position.
...
for (let i = 0; i < 8; i++) {
  const a = i * 9, b = 7 + i * 7;
  if (a < 32) DIAG_A1H8_LO |= (1 << a); else DIAG_A1H8_HI |= (1 << (a - 32));
  if (b < 32) DIAG_A8H1_LO |= (1 << b); else DIAG_A8H1_HI |= (1 << (b - 32));
}
...
function pinRisk(piece, diagonalRay, openFiles, openDiags) {
  switch (piece) {
    case PIECES.PAWN:   return diagonalRay ? 10 : 0;
    case PIECES.KNIGHT: return 140;
...
function phaseOf(board) {
  ...
  return p >= PHASE_MAX ? 1 : p / PHASE_MAX;
}

function openFileCount(board) {
  ...
  let n = 0;
  for (let f = 0; f < 8; f++) {
    if (((lo & FILE_LO[f]) | (hi & FILE_HI[f])) === 0) n++;
  }
  return n;
}

function openDiagCount(board) {
  const wp = board.bbPieces[WHITE_IDX][PIECES.PAWN], bp = board.bbPieces[BLACK_IDX][PIECES.PAWN];
  const lo = (wp.low | bp.low) | 0, hi = (wp.high | bp.high) | 0;
  let n = 0;
...
//
// Gravity (`h += bonus - h*|bonus|/MAX`) is self-normalising: a move near the
// ceiling gains almost nothing from another success.
// ═══════════════════════════════════════════════════════════════════════════
export class HistoryTable {
  ...
  update(move, depth, isGoodMove) {
    ...
    const capped = raw > HISTORY_BONUS_CAP ? HISTORY_BONUS_CAP : raw;
    const bonus = isGoodMove ? capped : -(capped >> 1);
    const h = this.h[idx];
...
export class CounterMoveTable {
  ...
  getEncoded(lastMove, lastMoverIdx) {
    if (lastMove === null || lastMove.piece === PIECES.NONE) return 0;
  ...
  update(lastMove, lastMoverIdx, counterMove) {
    if (lastMove === null || lastMove.piece === PIECES.NONE || counterMove.capturedPiece !== null) return;
...
export class MoveOrderer {
  constructor(config = {}) {
    // ── Tables (null when the feature is disabled) ──
    ...
    // ── Pre-allocated scoring context. Fixed shape. NOT reentrant. ──
  ...
  _prepareContext(ctx, board, color, ply, ttMove, lastMove, bookHints, bookPick) {
    ...
    ctx.openFiles = ctx.softActive ? openFileCount(board) : 0;
    ctx.openDiags = ctx.softActive ? openDiagCount(board) : 0;
  }
  ...
  _quietScore(move, ctx, board) {
    ...
    // History + positional signal, ADDED (not exclusive).
    let q = this.history !== null ? this.history.getScore(move) : 0;
  ...
  addKiller(move, ply) {
    if (this.killers !== null) this.killers.add(move, ply);
  }

  updateHistory(move, depth, isGoodMove) {
    if (this.history !== null) this.history.update(move, depth, isGoodMove);
  }

  updateCounterMove(lastMove, lastMoverIdx, counterMove) {
    this.counters.update(lastMove, lastMoverIdx, counterMove);
  }

  prepareNewSearch() {
    if (this.history !== null) this.history.age();
  }

  clear() {
    if (this.killers !== null) this.killers.clear();
