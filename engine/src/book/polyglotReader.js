 ...
 * 
 * Random numbers sourced from PolyGlot 1.4 hash.cpp by Fabien Letouzey
 * Verified: starting position hash = 463b96181691fc9c
 */
...
// ============================================================================
// Official Polyglot Zobrist random numbers
// Source: PolyGlot 1.4 by Fabien Letouzey (hash.cpp Random64[])
//
// Layout (781 values total):
//   [0..767]   = piece-square keys: 12 piece types * 64 squares
//   [768..771] = castling keys: white-K, white-Q, black-k, black-q
//   [772..779] = en passant file keys: files a through h
//   [780]      = side-to-move key (XOR when white to move)
//
// Piece ordering (each gets 64 consecutive entries):
//   0=BlackPawn, 1=WhitePawn, 2=BlackKnight, 3=WhiteKnight,
//   4=BlackBishop, 5=WhiteBishop, 6=BlackRook, 7=WhiteRook,
//   8=BlackQueen, 9=WhiteQueen, 10=BlackKing, 11=WhiteKing
//
// Square ordering within each piece block:
//   index = rank*8 + file  (a1=0, b1=1, ..., h1=7, a2=8, ..., h8=63)
// ============================================================================
...
function parseFen(fen) {
  ...
  return { board, activeColor, castling, epSquare };
}

export function generatePolyglotHash(fen) {
  const { board, activeColor, castling, epSquare } = parseFen(fen);

  let hash = 0n;

  // Hash pieces
  for (let fenRankIdx = 0; fenRankIdx < 8; fenRankIdx++) {
    const polyglotRank = 7 - fenRankIdx;

    for (let file = 0; file < 8; file++) {
      const piece = board[fenRankIdx][file];
      if (piece) {
        const pieceKey = piece.color + piece.type;
        const pieceType = PIECE_TYPE_MAP[pieceKey];

        if (pieceType !== undefined) {
          const square = polyglotRank * 8 + file;
          const randomIndex = 64 * pieceType + square;
          hash ^= Random64[randomIndex];
        }
      }
    }
  }
  ...
  // En passant
  if (epSquare !== '-') {
    const epFile = FILES.indexOf(epSquare[0]);
    const capturingPawnRank = activeColor === 'w' ? 4 : 3;
    const fenRowIdx = 7 - capturingPawnRank;
    let hasCapturingPawn = false;

    if (epFile > 0) {
      const leftPiece = board[fenRowIdx][epFile - 1];
      if (leftPiece && leftPiece.type === 'p' && leftPiece.color === activeColor) {
        hasCapturingPawn = true;
      }
    }

    if (epFile < 7) {
      const rightPiece = board[fenRowIdx][epFile + 1];
      if (rightPiece && rightPiece.type === 'p' && rightPiece.color === activeColor) {
        hasCapturingPawn = true;
      }
    }
...
function decodeMove(moveInt) {
  const toFile = moveInt & 0x7;
  const toRank = (moveInt >> 3) & 0x7;
  const fromFile = (moveInt >> 6) & 0x7;
  const fromRank = (moveInt >> 9) & 0x7;
  const promotion = (moveInt >> 12) & 0x7;

  const from = FILES[fromFile] + (fromRank + 1);
  const to = FILES[toFile] + (toRank + 1);

  const promotionChars = { 1: 'n', 2: 'b', 3: 'r', 4: 'q' };
  const promoChar = promotionChars[promotion] || '';
...
export class PolyglotBook {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        if (buffer.length % 16 !== 0) {
          reject(new Error(`Invalid Polyglot book: size not multiple of 16`));
          return;
        }

        const entryCount = buffer.length / 16;
        ...
        for (let i = 0; i < entryCount; i++) {
          ...
          const learn = buffer.readUInt32BE(offset + 12);

          if (moveInt === 0) continue;

          const move = decodeMove(moveInt);

          if (!this.entries.has(keyHex)) {
            this.entries.set(keyHex, []);
          }

          this.entries.get(keyHex).push({ move, weight, learn });
        }
  ...
  find(fen) {
    ...
    const hash = generatePolyglotHash(fen);
    const keyHex = hash.toString(16).padStart(16, '0');
